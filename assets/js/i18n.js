...
export const I18n = (function () {
    ...
    const messages = {
        'zh-CN': {
            ...
            // 首页特性区
            'features.title': '核心特性',
            'features.eventdriven.code': '@message.on_message()\nasync def handler(event):\n    text = event.get_text()\n    await event.reply(f"收到: {text}")',
            ...
            'features.modular.desc': '模块以独立 PyPI 包分发，支持懒加载、热更新、依赖拓扑排序与优先级控制',
            'features.modular.code': '# pip install ErisPulse-MyModule\n# or: epsdk install MyModule\n\nclass Main(BaseModule):\n    @staticmethod\n    def get_load_strategy():\n        return ModuleLoadStrategy(\n            lazy_load=True,   # 首次访问时才初始化\n            priority=10        # 数值越大越先加载\n        )\n\n    async def on_load(self, event):\n        self.logger.info("模块已就绪")',
        ...
        'en': {
            ...
            'features.modular.desc': 'Modules distributed as independent PyPI packages with lazy loading, hot updates, dependency topological sort, and priority control',
            ...
            'features.parallel.desc': 'Same-priority handlers run in parallel, different priorities run serially, Copy-On-Write zero overhead, with interrupt support',
            'features.parallel.code': '@message.on_message(priority=10)\nasync def spam_filter(event):\n    if is_spam(event.get_text()):\n        event.mark_processed()  # Interrupt lower priority\n\n# Same priority → parallel, zero-copy\n@message.on_message(priority=0)\nasync def handler_a(event):\n    event[\'result_a\'] = await process_a()\n\n@message.on_message(priority=0)\nasync def handler_b(event):\n    event[\'result_b\'] = await process_b()',
            'features.dashboard.title': 'Dashboard',
            'features.dashboard.desc': 'Real-time monitoring of adapter status, module loading progress, and bot online state — global visibility at a glance',
            'features.hooks.title': 'Lifecycle Hooks',
            'features.hooks.desc': 'Inject custom logic at every key stage of framework execution: module loading, adapter start/stop, bot online/offline — fully observable and responsive',
        ...
        'zh-TW': {
            ...
            'features.aicoding.code': '# 將 ErisPulse 物料投餵給 AI\n# 即可直接生成可用模組\n\nclass AIModule(BaseModule):\n    async def on_load(self):\n        self.logger.info("AI 生成就緒")',
            ...
            'features.modular.desc': '模組以獨立 PyPI 套件分發，支援懶載入、熱更新、依賴拓撲排序與優先級控制',
            ...
            'features.interactive.desc': '內建確認、選擇選單、表單收集、多輪對話等互動原語，輕鬆建構複雜互動流程',
            'features.parallel.title': '並行事件處理',
            'features.parallel.desc': '同優先級處理器並行執行，不同優先級串行排程，Copy-On-Write 零開銷，支援中斷控制',
            'features.parallel.code': '@message.on_message(priority=10)\nasync def spam_filter(event):\n    if is_spam(event.get_text()):\n        event.mark_processed()  # 中斷後續\n\n# 同優先級 → 並行執行，零拷貝\n@message.on_message(priority=0)\nasync def handler_a(event):\n    event[\'result_a\'] = await process_a()\n\n@message.on_message(priority=0)\nasync def handler_b(event):\n    event[\'result_b\'] = await process_b()',
            'features.dashboard.title': '儀表盤',
            'features.dashboard.desc': '即時監控適配器狀態、模組載入進度與 Bot 上線情況，全域執行狀態一目瞭然',
            'features.hooks.title': '生命週期鉤子',
            'features.hooks.desc': '在框架執行的每個關鍵階段插入自訂邏輯：模組載入、適配器啟停、Bot 上下線，全程可監聽可回應',
        ...
        'ja': {
            ...
            'features.aicoding.code': '# ErisPulse のドキュメントを AI に読み込ませる\n# すぐに使えるモジュールを生成\n\nclass AIModule(BaseModule):\n    async def on_load(self):\n        self.logger.info("AI モジュール準備完了")',
            ...
            'features.modular.desc': 'モジュールは独立 PyPI パッケージとして配布され、遅延読み込み、ホットアップデート、依存関係トポロジカルソート、優先度制御をサポート',
            ...
            'features.parallel.desc': '同じ優先度のハンドラーは並列実行、異なる優先度は直列スケジューリング、Copy-On-Write でゼロオーバーヘッド、中断制御をサポート',
            'features.parallel.code': '@message.on_message(priority=10)\nasync def spam_filter(event):\n    if is_spam(event.get_text()):\n        event.mark_processed()  # 低優先度を中断\n\n# 同優先度 → 並列実行、ゼロコピー\n@message.on_message(priority=0)\nasync def handler_a(event):\n    event[\'result_a\'] = await process_a()\n\n@message.on_message(priority=0)\nasync def handler_b(event):\n    event[\'result_b\'] = await process_b()',
            'features.dashboard.title': 'ダッシュボード',
            'features.dashboard.desc': 'アダプター状態、モジュール読み込み進捗、Bot のオンライン状況をリアルタイム監視 — グローバル状態を一目で把握',
            'features.hooks.title': 'ライフサイクルフック',
            'features.hooks.desc': 'フレームワーク実行の各重要段階にカスタムロジックを挿入：モジュール読み込み、アダプター開始/停止、Bot オンライン/オフライン — 全プロセスを監視・応答可能',
        ...
        'ru': {
            ...
            'features.modular.desc': 'Модули распространяются как независимые PyPI-пакеты с поддержкой ленивой загрузки, горячих обновлений, топологической сортировки зависимостей и управления приоритетами',
            ...
            'features.parallel.desc': 'Обработчики с одинаковым приоритетом выполняются параллельно, разные приоритеты — последовательно, Copy-On-Write с нулевыми накладными расходами, поддержка прерываний',
            ...
            'features.dashboard.desc': 'Мониторинг в реальном времени состояния адаптеров, прогресса загрузки модулей и статуса ботов — полная видимость работы системы',
            'features.hooks.title': 'Хуки жизненного цикла',
            'features.hooks.desc': 'Вставляйте пользовательскую логику на каждом ключевом этапе работы фреймворка: загрузка модулей, запуск/остановка адаптеров, подключение/отключение ботов — всё отслеживаемо и управляемо',
